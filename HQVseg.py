...
class Model(object):
    ...
    def visualize(self, window=None, figsize=None):
        ...
        # populate masks from HQV object lists if available
        try:
            ...
            print('cannot populate HQV masks')
            imsk = np.zeros(self.source.slog10q.shape, dtype='bool')
            smsk = imsk
            bmsk = imsk
            vmsk = imsk
            hmsk = imsk
...
def null_finder(nx,ny,nz,xv,yv,zv,B_x1,B_y1,B_z1, tolerance=None, max_steps=None):

    """
    Created on Tue Jul 16 16:45:56 2019
    
    @author: federicachiti

    adapted by R.B.Scott for inclusion in HQVseg -- 28/07/19
    """
    ...
    # k is the array of trilinear coefficients
    def quad_roots (k):
    ...
    # tri_v is an array of the trilinear coefficients associated with the v coordinate
    ...
    # nx, ny, nz are represent the resolution of the grid
    # xv, yv, zv are the coordinate in the x, y and z directions
    # B_x1, B_y1, B_z1 are the components of the B field
    # tolerance is a parameter set to determine the convergence of the solution



    # begin main routine: initialize variables
    ...
    for ind in ind_list:
        ...
        # if not (check_sign(bxx) and check_sign(byy) and check_sign(bzz)):
            ...
            if max_steps is None: max_steps = 10
                            ...
                            if ((xv[i] <= xs <= xv[i+1]) and (yv[j] <= ys <= yv[j+1]) and (zv[k] <= zs <= zv[k+1])):
                                if new_B[0] < tolerance*np.mean([B1,B2,B3,B4,B5,B6,B7,B8]):
                                    ...
                                    if zs <= zv[-2]: # this excludes the null points located on the null line that goes around the two outermost shells
                                        this_null = {'i':i, 'j':j, 'k':k, 'n': num_nulls, 'x': xs, 'y': ys, 'z': zs, \
                                                     'B': new_B[0], 'Error' : np.array([err_rel_grid[-1], err_rel_sol[-1]]).max(), 'iter' : n_steps }
